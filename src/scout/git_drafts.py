...
def _call_graph_summary_for_scope(
    ...
    for e in edges:
        ...
        in_scope = (
            not scope or file_from.startswith(scope) or file_from == scope.rstrip("/")
        )
        if not in_scope:
            continue
...
def assemble_pr_description_from_docs(
    ...
    *,
    ...
    for tldr_path in tldr_files:
        stem = tldr_path.stem.removesuffix(
            ".tldr"
        )  # e.g. doc_generation.py from doc_generation.py.tldr
        try:
    ...
    for file_path in doc_files:
        ...
        if draft_path.exists():
            try:
                ...
                sections.append(
                    content if content else f"[{path.name}]: No draft available"
                )
            except OSError:
