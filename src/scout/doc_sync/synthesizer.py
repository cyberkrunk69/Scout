...
from scout.doc_sync.ast_facts import ModuleFacts
...
OUTPUT (prose only — no JSON, no disclaimers):"""
    ...
    async def synthesize_tldr_async(self, facts: ModuleFacts) -> Tuple[str, float]:
        ...
        facts_md = self._facts_to_markdown(
            facts
        )  # include_enrichment=True — docstrings are truth
    ...
    def _fallback_from_facts(self, facts: ModuleFacts) -> str:
        ...
        for name, fact in sorted(facts.symbols.items()):
            if fact.type == "constant":
                usage = (
                    f" (used at lines {', '.join(map(str, fact.used_at))})"
                    if fact.used_at
                    else ""
                )
                ...
                lines.append(
                    f"- `{name}`: class with methods {', '.join(fact.methods)}"
                )
        return "\n".join(lines)
...
REWRITTEN (fluent prose WITH semantic precision):"""
    ...
    async def synthesize_tldr_async(self, facts: ModuleFacts) -> Tuple[str, float]:
        ...
        phase2 = result2.content.strip()

        cost = result1.cost_usd + result2.cost_usd
...
class ReasoningDocSynthesizer(ConstrainedDocSynthesizer):
    ...
    async def synthesize_tldr_async(self, facts: ModuleFacts) -> Tuple[str, float]:
        ...
        """
        from scout.big_brain import GEMINI_MODEL_FLASH, call_big_brain_async
        from scout.llm.router import call_llm
        ...
        facts_md = self._facts_to_markdown_rich(facts)
        phase1_prompt = RichDocSynthesizer.PHASE_1_PROMPT.format(
