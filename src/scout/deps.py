...
class DependencyNode:
    ...
    depends_on: Set[SymbolRef] = field(
        default_factory=set
    )  # forward edges: what I import/call
    depended_by: Set[SymbolRef] = field(
        default_factory=set
...
def _extract_symbols_from_scope(scope: Path, repo_root: Path) -> List[Any]:
    ...
    """
    # TODO: Phase 2 - extract adapters module
    # from scout.adapters.python import PythonAdapter
    # adapter = PythonAdapter()
    return []  # Stub - will be implemented when adapters are extracted
...
class DependencyGraph:
    ...
    async def build_for_scope(self, scope: Path, force: bool = False) -> None:
        ...
        for sym in symbols:
            ...
            for dep_path, dep_name in sym.dependencies:
                dep_rel_path = (
                    dep_path.relative_to(self.repo_root)
                ...
                dep_node = self.nodes.setdefault(
                    str(dep_ref), DependencyNode(dep_ref, "")
                )
    ...
    def invalidate_cascade(self, changed_files: List[Path]) -> Set[SymbolRef]:
        ...
        for path in changed_files:
            ...
            stale_refs = [
                node.ref for node in self.nodes.values() if node.ref.path == rel_path
            ]
        ...
        if affected:
            logger.info(
                f"Invalidation cascade triggered: {len(affected)} symbols marked stale"
            )
            self._save_cache()
        else:
    ...
    def get_stats(self) -> dict:
        ...
        orphaned = sum(
            1 for n in self.nodes.values() if not (self.repo_root / n.ref.path).exists()
        )
        return {
            "total": total,
            "stale": stale,
    ...
    def _load_cache(self) -> None:
        ...
        try:
            ...
            if data.get("version", 0) != GRAPH_VERSION:
                logger.info(
                    f"Dependency graph cache version mismatch ({data.get('version')} != {GRAPH_VERSION}) — rebuilding"
                )
                return
            ...
            for ref_str, node_data in data["nodes"].items():
                self.nodes[ref_str] = DependencyNode(
                    ...
                    {
                        SymbolRef.from_string(d)
                        for d in node_data.get("depended_by", [])
                    },
                    (
                        datetime.fromisoformat(node_data["invalidated_at"])
                        if node_data.get("invalidated_at")
                        else None
                    ),
