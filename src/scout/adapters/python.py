...
from scout.adapters.base import LanguageAdapter, SymbolTree
...
def _extract_types_from_node(node: ast.AST) -> List[str]:
    ...
    def _name_from_annotation(ann: ast.expr | None) -> str | None:
        ...
        if isinstance(ann, ast.Attribute):
            return (
                f"{_name_from_annotation(ann.value)}.{ann.attr}"
                if isinstance(ann.value, ast.Name)
                else ann.attr
            )
        if isinstance(ann, ast.Subscript):
            return (
                _name_from_annotation(ann.slice)
...
class PythonAdapter(LanguageAdapter):
    ...
    def parse(self, file_path: Path) -> SymbolTree:
        ...
        try:
            ...
            raise SyntaxError(e.msg, (e.filename, e.lineno, e.offset, e.text)) from e
        ...
        def process_class(cls: ast.ClassDef) -> SymbolTree:
            ...
            for item in cls.body:
                if isinstance(item, (ast.FunctionDef, ast.AsyncFunctionDef)):
                    method_children.append(process_callable(item, "method"))
