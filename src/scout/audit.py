...
Survives SIGKILL, power loss, existential dread.
Answers "what happened and how much did it cost?"
...
EVENT_TYPES = frozenset(
    {
        ...
        "roast_with_docs",
        "tool_output_saved",
        # LLM safeguard events
        "llm_retry",
        "llm_error",
        "llm_circuit_open",
        "budget_exceeded",
        "reservation_commit",
        # Node lifecycle events (for Command Console)
        "node_spawn",
        "node_complete",
        "node_failed",
        "tool_start",
...
class AuditLog:
    ...
    def log(
        ...
        *,
        ...
        raw_brief_path: Optional[str] = None,
        **kwargs: Any,
    ) -> None:
        """
        ...
        Args:
            event_type: One of nav|brief|cascade|validation_fail|budget|skip|
                trigger|tldr|deep|doc_sync
    ...
    def _parse_line(self, line: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            logger.warning(
                "AuditLog: skipping malformed line (corruption recovery): %s", e
            )
            return None
    ...
    def gate_metrics(
        ...
        # Pass: gate_compress with confidence, no reason
        # (or not parse_fail/api_error/stale_cascade)
        passes = [
            e
            for e in gate_compress
            if e.get("confidence") is not None
            and e.get("reason") not in ("parse_fail", "api_error", "stale_cascade")
        ]
        # Escalate: gate_compress stale_cascade + gate_escalate
        escalates_stale = [
            e for e in gate_compress if e.get("reason") == "stale_cascade"
        ]
    ...
    def flush(self) -> None:
        """Force flush and fsync to ensure events are persisted (e.g. before exit)."""
        with self._lock:
            if self._file is not None and not self._file.closed:
                try:
                    self._file.flush()
...
# Metrics aggregation for observability
class MetricsAggregator:
    """In-memory metrics aggregator for observability endpoint."""

    def __init__(self, audit_log: AuditLog, window_seconds: int = 60):
        """Initialize aggregator.

        Args:
            audit_log: AuditLog instance to aggregate from
            window_seconds: Aggregation window in seconds (default 60s)
        """
    ...
    def aggregate(self, force: bool = False) -> Dict[str, Any]:
        ...
        Args:
            force: Force re-aggregation even if cached
        ...
        with self._lock:
            ...
            if not force and self._cached_metrics and self._last_aggregation:
                age = (now - self._last_aggregation).total_seconds()
                if age < self._window_seconds:
                    return self._cached_metrics

            # Aggregate fresh metrics
            cutoff = now - timedelta(seconds=self._window_seconds)
            events = self._audit.query(since=cutoff)

            metrics = self._compute_metrics(events, now)
            self._cached_metrics = metrics
            self._last_aggregation = now
            return metrics

    def _compute_metrics(self, events: List[Dict[str, Any]], now: datetime) -> Dict[str, Any]:
        ...
        for event in events:
            ...
            # Track layer if present
            layer = event.get("layer")
            if layer:
                by_layer[layer] = by_layer.get(layer, 0) + 1

            # Track cost
            cost = event.get("cost") or event.get("cost_usd") or 0.0
            if cost:
                ...
                if ts:
                    ...
                    by_hour[hour_key] = by_hour.get(hour_key, 0.0) + float(cost)
            ...
            if research_id:
                sessions.add(f"research:{research_id}")

        return {
            "uptime_seconds": self._window_seconds,
            ...
            "session_list": list(sessions)[:50],  # Limit for response size
            "aggregated_at": now.isoformat(),
        }


# Global metrics aggregator instance (lazy init)
_metrics_aggregator: Optional[MetricsAggregator] = None
_metrics_lock = threading.Lock()


def get_metrics_aggregator(window_seconds: int = 60) -> MetricsAggregator:
