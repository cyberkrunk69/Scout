...
from scout.middle_manager import GateDecision
...
def _fallback_gate_whimsy(
    ...
    outcome_emoji = (
        "😎 PASS"
        if outcome == "pass"
        else "🚨 ESCALATE" if outcome == "escalate" else "❌ REJECT"
    )
    expansion = ""
    if expanded_symbols:
        display_syms = [
            s.split("::")[0] if "::" in s else s for s in expanded_symbols[:2]
        ]
...
def decision_to_whimsy_params(decision: GateDecision, cost_usd: float) -> dict:
    ...
    initial = (
        decision.initial_confidence if hasattr(decision, "initial_confidence") else conf
    )
    return {
        ...
        "outcome": (
            ...
            else "reject" if decision.decision == "reject" else "escalate"
        ),
        "cost_usd": cost_usd,
    }
...
from scout.ui.whimsy_data import DEFAULT_PHRASE_BANKS, load_user_phrase_bank
import hashlib
