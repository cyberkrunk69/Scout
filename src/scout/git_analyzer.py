...
def _run_git(
    ...
    try:
        ...
        stderr = e.stderr or ""
        logger.warning("Git command failed: %s (stderr: %s)", " ".join(cmd), stderr)
        raise
...
def get_changed_files(
    ...
    Args:
        ...
        diff_range: If set, use this range directly (e.g. "main..HEAD").
                    Takes precedence over base_branch.
    ...
    try:
        if diff_range:
            result = _run_git(
                ["diff", "--name-only", "--diff-filter=ACMRT", diff_range],
                cwd=repo_root,
            )
        elif base_branch:
            result = _run_git(
...
def get_base_branch(
    ...
    try:
        # 1. Try remote tracking: rev-parse --abbrev-ref --symbolic-full-name @{u}
        result = _run_git(
            ["rev-parse", "--abbrev-ref", "--symbolic-full-name", "@{u}"],
            cwd=repo_root,
        )
