...
Philosophy: Right-Size Tooling - deterministic validation before LLM
"""
...
class Validator:
    """Thin wrapper around validate_location for dependency injection with trust integration."""

    def __init__(self, repo_root: Path, trust_level: str = "normal"):
        self.repo_root = repo_root
        self.trust_level = trust_level
        self._orchestrator: Optional[TrustOrchestrator] = None
        self._initialized = False

    async def _ensure_orchestrator(self) -> "TrustOrchestrator":
        """Lazy initialization of trust orchestrator."""
        # TODO: Phase 2 - extract trust module
        # from scout.trust.orchestrator import TrustOrchestrator, TrustConfig
        raise ImportError("Trust orchestrator not yet available - Phase 2")
        # config = TrustConfig(strictness=self.trust_level)
        # self._orchestrator = TrustOrchestrator(self.repo_root, config=config)
        # await self._orchestrator.initialize()
        # self._initialized = True
        # return self._orchestrator

    async def validate(self, suggestion: dict) -> ValidationResult:
        """Validate with trust outcome tracking (async)."""
        result = validate_location(suggestion, self.repo_root)

        # Record outcome for trust learning
        try:
            orchestrator = await self._ensure_orchestrator()
            source_path = suggestion.get("file", "")

            await orchestrator.record_outcome(source_path, result.is_valid)
            ...
            pass  # Trust integration should not block validation

        return result

    def validate_sync(self, suggestion: dict) -> ValidationResult:
        """Validate without trust tracking (sync version for non-async contexts)."""
        return validate_location(suggestion, self.repo_root)

    async def shutdown(self) -> None:
        """Clean up trust orchestrator."""
