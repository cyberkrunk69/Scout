...
DEFAULT_CONFIG = {
    ...
    "doc_relationships": {
        "enabled": True,              # Master switch for relationship injection
        "include_callers": True,     # Functions that call this symbol
        "include_callees": True,      # Functions this symbol calls
        "include_inheritance": True,  # Base classes and subclasses
        "include_imports": True,      # Which modules import this symbol
        "include_examples": True,     # Usage snippets from real code
        "include_mermaid": True,      # Generate Mermaid diagrams for classes
        "max_related_items": 5,       # Truncation limit for relationship lists
    },
    "model_pricing": {
    ...
    "plan_locks": {
        ...
        "stale_hours": 1, # Stale lock threshold in hours
    },
    "plan_retention": {
        "archive_days": 7,  # Archive completed plans older than N days
        "delete_days": 30,  # Delete archived plans older than N days
    },
    "trust": {
        "enabled": True,
        "strictness": "normal",  # "permissive", "normal", "strict"
        "auto_repair_enabled": True,
        "auto_repair_threshold": 5,
        "min_nav_confidence": 70,
    },
    "validation": {
        "enabled": True,
        "stages": ["deterministic", "heuristic"],  # "llm_based" opt-in
    ...
    "quality_gates": {
        "enabled": False,  # Off by default, opt-in per tool
        "stages": ["unit", "integration"],  # "e2e" opt-in
        "auto_gate": False,  # If True, auto-submit outputs to gates
    },
    "budget": {
        "track_actual_costs": True,  # Enable real cost tracking
        "reservation_timeout_seconds": 30,
        "allow_overage_percent": 10,  # Allow 10% over-estimate
    },
    "graph": {
        "enabled": True,  # Enable relationship graph extraction
        "track_variables": True,  # Include variable refs
        "max_depth": 10,  # For trace_path
        "cache_ttl_seconds": 3600,
    },
    "research": {
        "intent_threshold": 0.7,  # Direct research routing threshold
        "llm_fallback_threshold": 0.5,  # LLM decision threshold
        "max_agents": 5,  # Max parallel research agents
        "agent_timeout_seconds": 60,  # Timeout per agent
        "max_cost_per_session": 0.50,  # Budget cap per research session
    },
    "improvement": {
        "intent_threshold": 0.7,  # Direct improvement routing threshold
        "llm_fallback_threshold": 0.5,  # LLM decision threshold
        "max_agents": 3,  # Max parallel code analysis agents
        "agent_timeout_seconds": 120,  # Timeout per agent
        "max_cost_per_session": 1.00,  # Budget cap per improvement session
    },
    "resilience": {
        "session_persistence_path": ".scout/browser_sessions.json",  # Path for session persistence
        "session_checkpoint_interval_seconds": 30,  # How often to checkpoint active sessions
        "chrome_health_check_interval_seconds": 10,  # How often to check Chrome process health
        "chrome_restart_max_attempts": 3,  # Max attempts to restart Chrome
        "chrome_restart_delay_seconds": 5,  # Delay between restart attempts
        "retry_base_delay_seconds": 1.0,  # Base delay for exponential backoff
        "retry_max_delay_seconds": 60.0,  # Max delay cap for exponential backoff
        "retry_jitter_factor": 0.1,  # Jitter as fraction of base delay
    },
}
...
ENV_TO_CONFIG = {
    ...
    "SCOUT_MODEL": ("model_pricing", "default_model", str),
    "SCOUT_INPUT_PRICE": ("model_pricing", "input_price", float),
    "SCOUT_OUTPUT_PRICE": ("model_pricing", "output_price", float),
    "SCOUT_PLAN_LOCK_TIMEOUT": ("plan_locks", "timeout", int),
    "SCOUT_PLAN_STALE_LOCK_HOURS": ("plan_locks", "stale_hours", int),
    "SCOUT_PLAN_ARCHIVE_DAYS": ("plan_retention", "archive_days", int),
    "SCOUT_PLAN_DELETE_DAYS": ("plan_retention", "delete_days", int),
    "SCOUT_GRAPH_ENABLED": ("graph", "enabled", bool),
    "SCOUT_GRAPH_TRACK_VARIABLES": ("graph", "track_variables", bool),
    "SCOUT_GRAPH_MAX_DEPTH": ("graph", "max_depth", int),
    "SCOUT_GRAPH_CACHE_TTL": ("graph", "cache_ttl_seconds", int),
    "SCOUT_DOC_RELATIONSHIPS_ENABLED": ("doc_relationships", "enabled", bool),
    "SCOUT_DOC_INCLUDE_CALLERS": ("doc_relationships", "include_callers", bool),
    "SCOUT_DOC_INCLUDE_CALLEES": ("doc_relationships", "include_callees", bool),
    "SCOUT_DOC_INCLUDE_INHERITANCE": ("doc_relationships", "include_inheritance", bool),
    "SCOUT_DOC_INCLUDE_EXAMPLES": ("doc_relationships", "include_examples", bool),
    "SCOUT_DOC_INCLUDE_MERMAID": ("doc_relationships", "include_mermaid", bool),
    "SCOUT_DOC_MAX_RELATED": ("doc_relationships", "max_related_items", int),
}
...
class ScoutConfig:
    ...
    def __init__(self, search_paths: Optional[List[Path]] = None):
        ...
        paths = (
            self._default_search_paths() if search_paths is None else list(search_paths)
        )
        for p in paths:
    ...
    def effective_max_cost(self, file_path: Optional[Path] = None) -> float:
        ...
        user_val = _get_nested(self._raw, "limits", "max_cost_per_event")
    ...
    def should_process(
        ...
        if estimated_cost > HARD_MAX_COST_PER_EVENT:
            return False
        hour_budget = float(self._raw.get("limits", {}).get("hourly_budget") or 1.0)
    ...
    def to_dict(self) -> dict:
        ...
        return {
            ...
            "model_pricing": dict(self._raw.get("model_pricing", {})),
            "ui": dict(self._raw.get("ui", {})),
            "validation": dict(self._raw.get("validation", {})),
            "quality_gates": dict(self._raw.get("quality_gates", {})),
            "budget": dict(self._raw.get("budget", {})),
            "graph": dict(self._raw.get("graph", {})),
            "doc_relationships": dict(self._raw.get("doc_relationships", {})),
            "resilience": dict(self._raw.get("resilience", {})),
            "hard_caps": {
    ...
    def get_trust_config(self) -> Dict[str, Any]:
        ...
        return dict(self._raw.get("trust", {}))

    def get_validation_config(self) -> Dict[str, Any]:
        """Get validation configuration."""
        return dict(self._raw.get("validation", {}))

    def get_quality_gates_config(self) -> Dict[str, Any]:
        """Get quality gates configuration."""
        return dict(self._raw.get("quality_gates", {}))

    def get_budget_config(self) -> Dict[str, Any]:
        """Get budget configuration."""
        return dict(self._raw.get("budget", {}))

    def get_graph_config(self) -> Dict[str, Any]:
        """Get graph configuration."""
        return dict(self._raw.get("graph", {}))

    def get_doc_relationships_config(self) -> Dict[str, Any]:
        """Get doc_relationships configuration."""
        return dict(self._raw.get("doc_relationships", {}))

    def get_resilience_config(self) -> Dict[str, Any]:
        """Get resilience configuration for browser sessions and retry logic."""
        return dict(self._raw.get("resilience", {}))
